...
func (qs QueryServiceEvent) GetList(q event.GetEventListQueryParam) (events []event.EventEmbed, err error) {
	// MySQLサーバーに接続
	d, err := Open()
	if err != nil {
		return
	...
	query += " ORDER BY 1, 2 IS NULL ASC"
	...
	for r.Next() {
		...
		if tmpEvent == nil || tmpEvent.Id != eId {
			...
			if tmpEvent != nil {
				...
				// 読み込み用変数をクリア
				tmpEvent = nil
				tmpDocuments = []event.EventDocument{}
			}
			// Scanしたフィールドを代入
